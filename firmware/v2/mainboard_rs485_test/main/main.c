...
//
// Wire pins per hardware/v2/mainboard/mainboard_sch.py: TX=IO17, RX=IO16,
// DE=IO4 (SP3485, half-duplex, 1 Mbps). Frame format and CRC8 must stay
// byte-for-byte identical to firmware/v2/leg/src/protocol.c — see
...
static int build_pull_frame(char *out, size_t out_sz, uint8_t flags, float coxa, float femur, float tibia)
{
    int n = snprintf(out, out_sz, ">%d,%02X,%.1f,%.1f,%.1f", TEST_LEG_ADDR, flags, coxa, femur, tibia);
...
static void run_benchmark(const char *label, int64_t timeout_us, int n, uint8_t flags)
{
    ...
    int flen = build_pull_frame(frame, sizeof(frame), flags, 0.0f, 0.0f, 0.0f);
...
void app_main(void)
{
    ...
    while (1) {
        ...
        int flen = build_pull_frame(frame, sizeof(frame), 0x01, angle, angle, angle);
        ...
        if (llen > 0 && validate_response(line)) {
            ESP_LOGI(TAG, "leg %d OK: %s", TEST_LEG_ADDR, line);
        } else {
            ESP_LOGW(TAG, "no response (timeout) — sent: %s", frame);
        }
